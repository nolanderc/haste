...
impl<K, V, const SHARDS: usize> ShardMap<K, V, SHARDS> {
        ...
        K: Hash + Eq,
    {
        ...
        match table.get_mut(hash, Entry::eq_fn(&entry.key)) {
            Some(old) => Some(std::mem::replace(old, entry).value),
            None => {
                table.insert_entry(hash, entry, Entry::hash_fn(&self.hasher));
                None
            }
        }
    }
        ...
        K: Hash + Eq,
    {
        ...
        ReadGuard::try_map(table, |table| table.get(hash, Entry::eq_fn(key))).ok()
    }
}

impl<K, V> Entry<K, V> {
    #[inline(always)]
    fn hash_fn(builder: &BuildHasherDefault) -> impl Fn(&Self) -> u64 + '_
    where
        K: Hash,
    {
        move |entry| hash_one(&entry.key, builder)
    }

    #[inline(always)]
    fn eq_fn(key: &K) -> impl Fn(&Self) -> bool + '_
    where
        K: Eq,
    {
